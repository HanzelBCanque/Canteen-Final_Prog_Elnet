(() => {
    ...
    async function submitLogin() {
        ...
        if (usernameInput) state.username = usernameInput.value.trim().toUpperCase();
        if (passwordInput) state.password = passwordInput.value.trim();

        if (!state.username || !state.password) {
            state.loginError = "Please enter both username and password";
            render();
            return;
        }
        ...
        try {
            const employee = await fetchJson("/api/staff/login", {
                method: "POST",
                body: JSON.stringify({
                    // Updated keys to match the new C# LoginRequest
    ...
    function renderLandingScreen() {
      ...
      <section class="landing-screen">
        ...
        ${!state.loginOpen ? `<button class="landing-login" data-action="open-login">LOG IN</button>` : ""}
      </section>
    `;
    }
    ...
    function renderLoginModal() {
        return `
    <div class="modal-backdrop" style="position: fixed; inset: 0; background: rgba(255,255,255,0.01); z-index: 100; display: flex; align-items: center; justify-content: flex-start; padding-left: 5vw;">

      <div style="background: #cfe2ff; width: 400px; padding: 40px; border-radius: 40px; border: 1px solid #999; box-shadow: 0 4px 15px rgba(0,0,0,0.1); text-align: center;">

        <img src="https://upload.wikimedia.org/wikipedia/en/thumb/0/0e/University_of_Cebu_Logo.svg/1200px-University_of_Cebu_Logo.svg.png" 
             alt="UC Logo" style="width: 180px; margin-bottom: 30px;" />

        ${state.loginError ? `<div class="error-box" style="margin-bottom: 1rem; font-size: 0.85rem; padding: 0.5rem;">${escapeHtml(state.loginError)}</div>` : ""}

        <div style="text-align: left; margin-bottom: 20px;">
          <label style="display: block; font-weight: 800; font-size: 1.2rem; margin-bottom: 8px;">Username</label>
          <input class="input" name="username" type="text" placeholder="Enter your username..." value="${escapeHtml(state.username)}" 
                 style="width: 100%; height: 50px; border-radius: 25px; border: 1px solid #999; padding: 0 20px; background: #f3f3f3;" />
        </div>

        <div style="text-align: left; margin-bottom: 35px;">
          <label style="display: block; font-weight: 800; font-size: 1.2rem; margin-bottom: 8px;">Password</label>
          <input class="input" name="password" type="password" placeholder="Enter your password..." value="${escapeHtml(state.password)}" 
                 style="width: 100%; height: 50px; border-radius: 25px; border: 1px solid #999; padding: 0 20px; background: #f3f3f3;" />
        </div>

        <button class="btn-main" style="background: #4a74b4; color: white; width: 180px; height: 50px; border-radius: 25px; font-size: 1.2rem; font-weight: 800;" data-action="submit-login">
          Sign in
        </button>

        <button class="btn-light" style="display: block; margin: 15px auto 0; background: transparent; border: none; font-weight: 800; color: #666;" data-action="close-login">
          Cancel
        </button>
      </div>
  ...
  `;
    ...
    appRoot.addEventListener("input", event => {
        ...
        if (target.name === "username") {
            state.username = target.value.trim().toUpperCase(); // This saves ADMIN123
            state.loginError = "";
        }
        if (target.name === "password") {
            state.password = target.value; // This saves ADMINPASS
            state.loginError = "";
        }
